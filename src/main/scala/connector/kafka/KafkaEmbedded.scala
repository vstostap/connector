...
/**
  ...
  */
class KafkaEmbedded(config: Properties = new Properties) {
  ...
  private val effectiveConfig = {
    ...
    c.load(this.getClass.getResourceAsStream("/broker.properties"))
    c.putAll(config)
  ...
  val zookeeperConnect = {
    ...
    zkConnectLookup match {
      ...
      case _ =>
        println(s"zookeeper.connect is not configured -- falling back to default setting $defaultZkConnect")
        defaultZkConnect
    }
  }
  ...
  /**
    ...
    */
  def start() {
    println(s"Starting embedded Kafka broker at $brokerList (with ZK server at $zookeeperConnect) ...")
    kafka.startup()
    println(s"Startup of embedded Kafka broker at $brokerList completed (with ZK server at $zookeeperConnect)")
  }
  ...
  /**
    ...
    */
  def stop() {
    println(s"Shutting down embedded Kafka broker at $brokerList (with ZK server at $zookeeperConnect)...")
    kafka.shutdown()
    FileUtils.deleteQuietly(logDir)
    println(s"Shutdown of embedded Kafka broker at $brokerList completed (with ZK server at $zookeeperConnect)")
  }

  def createTopic(topic: String, partitions: Int = 1, replicationFactor: Int = 1, config: Properties = new Properties): Unit = {
    println(s"Creating topic { name: $topic, partitions: $partitions, replicationFactor: $replicationFactor, config: $config }")
